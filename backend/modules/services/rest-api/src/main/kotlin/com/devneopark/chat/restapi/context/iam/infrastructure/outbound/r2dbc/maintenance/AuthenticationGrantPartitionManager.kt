...
class AuthenticationGrantPartitionManager(
    ...
    private val temporalAdjuster: TemporalAdjuster,

    private val defaultTimezone: ZoneOffset,
    ...
    suspend fun ensurePartition(now: Instant) {
        val partitionWeekStart = now.atZone(defaultTimezone)
            .toLocalDate()
        ...
        val sql = """
            ...
            for values from ('${partitionWeekStart.atStartOfDay(defaultTimezone)}')
                    to ('${nextWeekStart.atStartOfDay(defaultTimezone)}')
        """.trimIndent()
     ...
     */
    @Transactional
    suspend fun ensureUpcomingPartitions(now: Instant) {
        val currentWeekStart = now.atZone(defaultTimezone)
            .toLocalDate()
            .with(temporalAdjuster)
        val lastRequiredWeekStart = now.plusMillis(refreshTokenTtlMillis)
     ...
     */
    @Transactional
    suspend fun dropPreviousWeekPartition(now: Instant) {
        val previousWeekStart = now.atZone(defaultTimezone)
            .toLocalDate()
            .with(temporalAdjuster)
            .minusWeeks(1)
        val format = DateTimeFormatter.BASIC_ISO_DATE
     ...
     */
    @Transactional
    suspend fun dropPartitionFromTwoWeeksAgo(now: Instant) {
        val twoWeeksAgoStart = now.atZone(defaultTimezone)
            .toLocalDate()
            .with(temporalAdjuster)
            .minusWeeks(2)
        val format = DateTimeFormatter.BASIC_ISO_DATE
